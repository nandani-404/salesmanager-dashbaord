...
const formatTruckerDate = (dateStr?: string | null) => {
  ...
  try {
    ...
    return `${day} ${monthName} ${year}, ${hourNum.toString().padStart(2, '0')}:${minuteStr} ${ampm}`;
  } catch {
    return dateStr;
  }
};
...
export default function TruckersPage() {
  ...
  useEffect(() => {
    const fetchTransporters = async () => {
      try {
        setLoading(true);
        let url = `${API_ENDPOINTS.dashboard.transporters}?page=${currentPage}`;
        if (filterStatus !== "all") {
          url += `&status=${filterStatus}`;
        }
        const response = await apiClient.get<TransportersResponse>(url);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        let url = `${API_ENDPOINTS.dashboard.transporters}?query=${encodeURIComponent(searchQuery)}`;
        if (filterStatus !== "all") {
          url += `&status=${filterStatus}`;
        }
        const response = await apiClient.get<TransportersResponse>(url);
        setSearchResults(response.data.transporters || []);
      } catch (err: any) {
        console.error("Search error:", err);
        setSearchError("Failed to search transporters");
        setSearchResults([]);
      } finally {
        setIsSearching(false);
      }
  ...
  const filteredTransporters = useMemo(() => {
    if (!data) return [];
    // Use search results if searching, otherwise use all transporters from API
    return searchQuery.trim() ? searchResults : data.transporters;
                  ...
                  <Card className="p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all bg-white rounded-xl relative h-full flex flex-col min-h-[380px]">
                    ...
                    <div className="flex items-start gap-4 mb-5 pb-5 border-b border-gray-100">
                      ...
                      <div className="flex-1 min-w-0 pt-1">
                        ...
                        <p className="text-gray-400 font-medium truncate text-xs mb-1">{t.unique_id}</p>
                        {t.Created_at && (
                          <p className="text-gray-400 font-medium truncate text-xs mb-2">{formatTruckerDate(t.Created_at)}</p>
                        )}
                        <Badge className={`border-none px-3 py-0.5 text-[10px] font-bold rounded-md ${t.verified_trucker_shipper === '1' || t.verified_trucker_shipper === 1
                  ...
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {t.Created_at && (
                          <p className="text-xs text-gray-400 truncate">{formatTruckerDate(t.Created_at)}</p>
                        )}
                        <div className="mt-1">
                          <Badge variant={
                            t.verified_trucker_shipper === '1' || t.verified_trucker_shipper === 1
        ...
        {data && (
          <div className="flex items-center justify-between border-t border-gray-100 pt-8 mt-10">
            <div className="text-sm text-[#64748b]">
              Showing <span className="font-semibold">{filteredTransporters.length}</span> results
            </div>
